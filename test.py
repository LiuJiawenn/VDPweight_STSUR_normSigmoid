...
def testing(model, videoList = list(range(44))):
    ...
    np.save('loss_history/0203true.npy', true_sur)
    np.save('loss_history/0203pred.npy', pred_sur)
...
if __name__ == '__main__':
    ...
    model = VDPWeightSTSUR(key_frame_nb=key_frame_nb, patch_per_frame=patch_per_frame)
    model.load_state_dict(torch.load('weights/0203model_weights_epoch89.pth'))
    model.to(device)
    true_sur,pred_sur = testing(model, videoList=videoList)

    true_sur = np.load('loss_history/0203true.npy').reshape(44,11)
    pred_sur = np.load('loss_history/0203pred.npy').reshape(44,11)
    true_list = []
    pred_list = []
    ...
    np.save('loss_history/0203predFited.npy',pred_list)